...
-- Select All Hotels with Ratings Greater Than 4
SELECT HotelId, Cust_Name, Location, Rating
FROM Hotels
WHERE Rating > 4;

-- Find All Rooms Available in a Specific Hotel
SELECT RoomID, RoomNumber, RoomType, PricePerNight
FROM Rooms
WHERE HotelID = 2 AND AvailabilityStatus = 'Available';

-- Get Total Amount of All Bookings Made by a Specific Customer
SELECT SUM(TotalAmount) AS TotalSpent
FROM Bookings
WHERE CustomerID = 1;

-- Retrieve All Amenities Offered by Hotels in Miami
SELECT a.AmenityType, a.Description
FROM Amenities a
JOIN Hotels h ON a.HotelID = h.HotelID
